...
**Core Value:** **One command brings up an entire portable office of multi-model coding agents — anywhere — and the owner controls them all from a phone.** If everything else breaks, this must work: `docker compose up -d` → log in → add agent against a repo → talk to it from phone or web.
